...
def get_SLU_datasets(config):
	...
	train_df = pd.concat([real_train_df]).reset_index()
	if not config.seq2seq:
...
class SLUDataset(torch.utils.data.Dataset):
	def __init__(self, df, base_path, Sy_intent, config, upsample_factor=1):
		...
		self.loader = torch.utils.data.DataLoader(self, batch_size=config.training_batch_size, num_workers=multiprocessing.cpu_count(), shuffle=False, collate_fn=CollateWavsSLU(self.Sy_intent, self.seq2seq))
